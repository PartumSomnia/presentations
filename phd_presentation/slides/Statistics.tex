...
\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.45cm,-0.0cm)}]current page.center){
        \parbox{0.65\textwidth}{
            \textbf{We observed:}
            \begin{itemize}
            \item neutrino absorption raises $\amd$, $\avd$;
            \item $\tilde{\Lambda}$ \& \mr{} give leading trends;
            \item simple polynomial perform (relatively) well;
            \item smooth fitting functions cannot reproduce oscillatory data;
            \item $\avd$ is correlated with $\tilde{\Lambda}$ for \DSrefset{};
            ...
            \item geometry: neutrion reabosroption -- more spherical ejecta;
            \item $M_{\rm disk}$: different extraction methods/times.
            \end{itemize}
    }};
}

\uncover<1->{ % <-> |
    \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.0cm,-0.7cm)}]current page.center){
        \parbox{0.60\textwidth}{
...
%            \label{tbl:fit:ejecta:chi2dofsall}
            \scalebox{0.65}{
                ...
                \hline\hline
                $\log_{10}(\md)$ & Datasets & Mean & Eq.\footcite{Dietrich:2016fpt} & Eq.\footcite{Kruger:2020gig} & $P_2^1(\tilde{\Lambda})$ & $P_2^2(q,\tilde{\Lambda})$ \\ \hline
                ...
                \hline\hline
                $\langle v_{\rm ej}\rangle$ & Datasets & Mean & Eq.\footcite{Dietrich:2016fpt} & & $P_2^1(\tilde{\Lambda})$ & $P_2^2(q,\tilde{\Lambda})$ \\ \hline
