...
\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-0.cm,2.2cm)}]current page.center){
        \parbox{1.1\textwidth}{
%            \begin{footnotesize}
...
%            \end{itemize}
            \textbf{Afterglow} is non-thermal (synchrotron) emission from ejecta moving through ISM\footcite{Sari:1997qe,Kumar:2014upa};
            \\
            \textbf{Kilonova afterglow} -- expected\footcite{Nakar:2011cw,Kathirgamaraju:2019xwu}, but so far there was no clear detection
%            \begin{itemize}
...
\uncover<2->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.5cm,-0.2cm)}]current page.center){
        \parbox{0.60\textwidth}{
            \textbf{\GRB{}} is a counterpart to \GW{} -- structured jet, observed off-axis\footcite{Troja:2017nqp,Hajela:2019mjy}.\\
...
\uncover<3->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.5cm,-2.0cm)}]current page.center){
        \parbox{0.60\textwidth}{
            Can kilonova afterglow explain this change in afterglow?
%            \begin{itemize}
%                \item change in microphysics parameters;
%                \item ISM density variation;
%                \item emergence of a new component;
...
\uncover<1->{ % <-> |
    \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.3cm,-1.5cm)}]current page.center){
        \parbox{0.5\textwidth}{
            %\includegraphics[height=6.3cm]{phd_figs/jet_ascenci20.pdf}
            \includegraphics[height=5.0cm]{figures/Hajela22_obs_pl_noNR.pdf}
...
%        }
        ...
        \uncover<1->{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(3.6cm,-1.6cm)}]current page.center){
                \parbox{0.6\textwidth}{
                    \includegraphics[height=5.0cm]{figures/Sari97_spectrum_slow_cool.pdf}
                    %                        Overall, an afterglow spectrum has three characteristic frequencies: 
                    %                        (i) $\nu_i$ (injection break) at which most of the injected electrons radiate 
                    %                        (ii) $\nu_c$ (cooling break) at which for electrons expansion timescale is equal 
                    %                        to the radiative cooling timesacle
...
\begin{frame}{Kilonova afterglow for \GRB{} rebrightening\footcite{Nedora:2021eoj}}%$^{\textcolor{gray}{\text{\cite{Nedora:2021eoj}}}}$} %% ---------- title 

\begin{tikzpicture}[overlay,remember picture]

\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-0.8cm,2.1cm)}]current page.center){
        \parbox{1.0\textwidth}{
            \begin{itemize}
            \item Overall agreement between models and observations.
            \item Very soft \acp{EOS} are less favored.
            \end{itemize}
    }};
}

\uncover<1->{ % <-> |
    \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(-4.2cm,-1.2cm)}]current page.center){
        \parbox{0.5\textwidth}{
            \includegraphics[height=5.5cm]{phd_figs/kn_afterglow/best_xray_obs_representative_all_eos.pdf}
    }};
}

\uncover<2->{ % <-> |
    \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.0cm,-1.2cm)}]current page.center){
        \parbox{0.5\textwidth}{
            \includegraphics[height=6.1cm]{phd_figs/kn_afterglow/scatter_lightcurve_tpeak_vs_lambda.pdf}
    }};
}
