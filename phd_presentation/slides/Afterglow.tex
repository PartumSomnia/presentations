...
\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.5cm,1.3cm)}]current page.center){
        \parbox{0.60\textwidth}{
            \begin{itemize}
                \item Synchrotron emission arising when a shock propagates through ISM$^{\textcolor{gray}{\text{\cite{Sari:1997qe}}}}$
                \item \GRB{} afterglow, structured jet, off-axis$^{\textcolor{gray}{\text{\cite{Hajela:2019mjy}}}}$
                \item expected \ac{kN} ejecta afterglow$^{\textcolor{gray}{\text{\cite{Kathirgamaraju:2019xwu}}}}$
            \end{itemize}
    }};
}
...
\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.5cm,-2.0cm)}]current page.center){
        \parbox{0.60\textwidth}{
            Change in afterglow in X-ray, $1243$ days post-merger. Possible explanations $^{\textcolor{gray}{\text{\cite{Hajela:2021faz}}}}$: 
            \begin{itemize}
...
%\end{tikzpicture}
%\end{frame}
\begin{frame}{Model: dynamics}  %% ---------- Intro/motivation 
    \begin{tikzpicture}[overlay,remember picture]
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-0.6cm,0.2cm)}]current page.center){
                \parbox{1.\textwidth}{
                    \textbf{Structure:} lateral, $\{\theta_i\}$, and velocity $\{\upsilon_i\}$; \\
                    \textbf{Dynamics:} Semi-analytic model (energy conservation) in 
                    this-shell (unifrom-shell) approximation; 
                    $ T^{\mu\nu} = (\rho' c^2 + p') u^{\mu}u^{\nu} + p' g^{\mu\nu} $ \\
                    adiabatic evolution; outflow "in cones": \\
                    $\{E_{i, 0},\Gamma_{i,0} \}=f(\Gamma,\theta)$; ($\Gamma=(1-\beta^2)^{-2}$); $\beta=\upsilon/c$; 
                    \vspace{-2mm}
                    \begin{equation*}
                    ...
                    From shock-conditions: $\rho_{\rm sh}$, $\beta_{\rm sh}$, $R_{\rm sh}$ \\
...
\begin{frame}{Model: radiation}  %% ---------- Intro/motivation 
    \begin{tikzpicture}[overlay,remember picture]
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.6cm,1.4cm)}]current page.center){
                \parbox{0.6\textwidth}{
                    \textbf{Shocks}$^{\textcolor{gray}{\text{\cite{Rezzolla:2013,Kumar:2014upa,Zhang:2018book,Nava:2013}}}}$
                    \begin{itemize}
            ...
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(4.6cm,1.3cm)}]current page.center){
                \parbox{0.6\textwidth}{
                    ...
                    \begin{equation*}\label{eq:method:gm_pl_only}
                        ...
                        %e' = \frac{E_{\rm int}'}{V'}
                    \end{equation*}
                    $\gamma_m' < \gamma_c' (>\gamma_c')$: slow (fast) cooling
            }};
...
%                        \end{cases}
        ...
        %        \uncover<1->{ % <-> |
            ...
            %        }
        \uncover<1->{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.8cm,-2.1cm)}]current page.center){
                \parbox{0.6\textwidth}{
