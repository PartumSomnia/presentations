...
%            \end{itemize}
            \textbf{Afterglow} non-thermal (Synchrotron) emission from ejecta moving through ISM\footcite{Sari:1997qe,Kumar:2014upa};
            \\
            \textbf{Kilonova afterglow} -- expected\footcite{Nakar:2011cw,Kathirgamaraju:2019xwu}, but so far no clear detection
%            \begin{itemize}
...
\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-0.8cm,2.1cm)}]current page.center){
        \parbox{1.0\textwidth}{
            ...
            \item Moderate $q$ and \ac{EOS} softness are preferred
            \end{itemize}
    }};
}
