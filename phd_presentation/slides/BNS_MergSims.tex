...
\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-0.0cm,-0.2cm)}]current page.center){
        \parbox{1.1\textwidth}{
            ...
            Numerical relativity code \wisky{}
            %$^\text{\textcolor{gray}{\citep{Radice:2013apa,Radice:2012cu,Radice:2013xpa,Radice:2013hxh,
                                 %Radice:2015nva,Radice:2016dwd,Radice:2018pdn,Radice:2020ids}}}$
            \footcite{Radice:2015nva,Radice:2018pdn,Radice:2020ids}
            includes: 
            \begin{itemize}
                \item general relativity,%Z4c formulation of $3+1$ decomposed, general relativity,
                %\item Conservative formulation of relativistic hydrodynamics,
                \item neutrino transport, %Neutrino radiation transport via Leakage $+$ M0 scheme,
                \item turbulent viscosity of magnetic origin,% via GRLES appraoch,
                \item microphsyical \ac{EOS} with finite temperature effects.
            \end{itemize}
            Overall, $37$ unique simulations targeted to \GW{}; some $100+$~ms long. % total of $76$,  \\
    ...
    \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(5.3cm,-0.6cm)}]current page.center){
        \parbox{0.5\textwidth}{
            ...
            %% \caption{3D distribution of angular momentum density flux $J_r$
                ...
                %%        (Adapted from \citet{Nedora:2019jhl})
                %%    }
            \hspace*{2.8mm}Spiral arms within the disk.%\footcite{Nedora:2019jhl}
    }};
}
\uncover<2->{ % <-> |
    \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.6cm,-0.4cm)}]current page.center){
        \parbox{0.5\textwidth}{
            ...
            \hspace*{10mm}Long-term remnant evolution.%\footcite{Nedora:2020pak}
...
\begin{frame}{Properties of the \swind{}\footcite{Nedora:2019jhl,Nedora:2020pak}}%$^\text{\textcolor{gray}{\citep{Nedora:2019jhl,Nedora:2020pak}}}$}
\begin{tikzpicture}[overlay,remember picture]
\uncover<1->{ % <-> |
    \uncover<1->{ % <-> |
        \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-4.5cm,0.5cm)}]current page.center){
            \parbox{0.48\textwidth}{
                %$E=-u_t-1 > 0$, 
                \begin{footnotesize}
                    \textbf{Unbound matter}: fluid specific energy $>0$:
                    \begin{itemize}
                        \item \textit{dynamical ejecta} -- geodesic criterion; %($E=-u_t-1 > 0$).
                        \item \textit{winds} -- Bernoulli criterion. %($E=-hu_t -1 > 0$).
                    \end{itemize}
                \end{footnotesize}
                \vspace{5mm}
                ...
                \textbf{Spiral-wave wind:}
                \begin{itemize}
                    \item in models with long-lived remnant,
                    \item larger if \ac{EOS} is softer or $q>1$.
                    %\item increases if turbulent viscosity is present
                \end{itemize}
%                $M_{\text{wind}}$ is larger for more extended disks. % that in turn depend on thermal pressure
    ...
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-4.5cm,-2.8cm)}]current page.center){
        \parbox{0.48\textwidth}{
            \textbf{Properties of \swind:}
            \begin{itemize}
                \item It is more massive, $M_{\rm wind}=f(t)$
%                \item $\amw\propto t_{\text{coll}} \gg \amd$,
                \item and less neutron rich, $\ayw>0.3$.
                %\item $\ayw > \ayd$ and $\ayw>0.3$.%$0.1\lesssim \ayw\lesssim0.4$ peak ${\simeq}0.35$
...
\uncover<1->{ % <-> |
    \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(5.65cm,1.2cm)}]current page.center){
        \parbox{1.0\textwidth}{
            %\includegraphics[height=4.5cm]{phd_figs/ejecta_postdyn/wind_hists_shared.pdf}
            \includegraphics[height=3.3cm]{phd_figs/ejecta_postdyn/ejecta_profiles_dd2_ls220_long.pdf}
    }};
}
\uncover<1->{ % <-> |
    \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(5.5cm,-2.5cm)}]current page.center){
        \parbox{1.0\textwidth}{
            %\includegraphics[height=4.5cm]{phd_figs/ejecta_postdyn/wind_hists_shared.pdf}
            \includegraphics[height=3.4cm]{phd_figs/ejecta_postdyn/hist_1D_dd2_ls220_long.pdf}
    }};
}
\end{tikzpicture}
\end{frame}
...
\begin{frame}{Fast component of the dynamical ejecta\footcite{Nedora:2021eoj}}%$^\text{\textcolor{gray}{\citep{Nedora:2021eoj}}}$ } %% ---------- title 
    \begin{tikzpicture}[overlay,remember picture]
        ...
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.2cm,1.8cm)}]current page.center){
                \parbox{0.65\textwidth}{
                    ...
                    \begin{itemize}
                        \item The fast ejecta is produced at \textit{core bounces}.
                        %\item absent in high-$q$ simulations
                        \item Mass is larger for $q{\sim}1$, low $\tilde{\Lambda}$.
        ...
        \uncover<1->{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.2cm,-1.56cm)}]current page.center){
                \parbox{0.5\textwidth}{
                    \includegraphics[height=5.0cm]{phd_figs/ejecta_dyn/fast_ejecta/scatter_ekej_vej06.pdf}
                    %\small{\textbf{Artist depiction of ejecta$^\text{\citep{Ascenzi:2020xqi}}$}}
            }};
        }
