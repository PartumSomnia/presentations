...
\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-0.0cm,-0.2cm)}]current page.center){
        \parbox{1.1\textwidth}{
            ...
            Numerical relativity codes, \eg, \wisky{} code
    ...
    \uncover<1->{ % <-> |
        \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-4.5cm,0.5cm)}]current page.center){
            \parbox{0.48\textwidth}{
                ...
                \begin{footnotesize}
                    \begin{itemize}
                        \item \textit{Dynamical ejecta}; ballistic; geodesic criterion ($E=-u_t-1 > 0$)
                        \item \textit{Winds}; steady-state; Bernoulli criterion ($E=-hu_t -1 > 0$)
                    \end{itemize}
                \end{footnotesize}
