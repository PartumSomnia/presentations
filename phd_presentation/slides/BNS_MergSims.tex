    ...
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.1cm,0.5cm)}]current page.center){
        \parbox{0.7\textwidth}{
            \begin{itemize}
                ...
                \item Remnant born with excess in $M_b$ and $J$ (HMNS)
...
\begin{frame}{Spiral-wave wind -- Properties$^\text{\textcolor{gray}{\citep{Nedora:2020pak}}}$}
\begin{tikzpicture}[overlay,remember picture]
\uncover<1->{ % <-> |
    \uncover<1->{ % <-> |
        \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-2.9cm,1.6cm)}]current page.center){
            \parbox{0.7\textwidth}{
                ...
                Bernoulli criterion ($E=-hu_t -1 > 0$, steady flow)
