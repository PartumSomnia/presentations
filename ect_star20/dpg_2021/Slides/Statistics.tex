...
\uncover<1->{ % <-> |
    \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.1cm,-2.0cm)}]current page.center){
        \parbox{0.65\textwidth}{
            ...
            \textbf{Goals}: asses
            \begin{itemize}
            \item the quality of fitting formulae (FF) via min. $\chi_{\nu}^{2}$;
...
\begin{frame}{Statistical analysis$^{\text{\citep{Nedora:2020qtd}}}$} %% ---------- title 
...
%            \label{tbl:fit:ejecta:chi2dofsall}
            \scalebox{0.55}{
                ...
                \hline\hline
                $\log_{10}(\md)$ & Datasets & Mean & Ref.~\citep{Dietrich:2016fpt} & Ref.~\citep{Kruger:2020gig} & $P_2^1(\tilde{\Lambda})$ & $P_2^2(q,\tilde{\Lambda})$ \\ \hline
                ...
                \hline\hline
                $\langle v_{\rm ej}\rangle$ & Datasets & Mean & Ref.~\citep{Dietrich:2016fpt} & & $P_2^1(\tilde{\Lambda})$ & $P_2^2(q,\tilde{\Lambda})$ \\ \hline
