...
%                    \end{itemize}
%                    \textbf{GRB + kilonova examples}\\
                    \textbf{GRB afterglow with NIR excess} 
            }};
        }
%        \uncover<1->{ % <-> |
        ...
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.2cm,1.7cm)}]current page.center){
                \parbox{0.65\textwidth}{
                    ...
                    depends on velocity and angular ejecta distributions; \\ 
                    ...
                    \hspace*{3mm}\textcolor{red}{*}\textit{GRB170817A late time light curve change\textcolor{red}{**}.  }
...
\begin{frame}{}  %% ---------- Intro/motivation 
    \begin{tikzpicture}[overlay,remember picture]
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-4.0cm,-0.1cm)}]current page.center){
                \parbox{0.57\textwidth}{
                    ...
                    \textbf{Unknowns}:
                    \begin{itemize}
                        \item will NR-informed kilonova ejecta ``break through'' observations-informed GRB one and when?
        ...
        \uncover<1->{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.7cm,3.75cm)}]current page.center){
                \parbox{0.35\textwidth}{
                    \textcolor{gray}{\footnotesize{VN+ (arxiv:2208.01558) }}
            }};
        }
    \end{tikzpicture}
\end{frame}
        ...
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-2.cm,1.2cm)}]current page.center){
                \parbox{0.80\textwidth}{
                    ...
                    \vspace{-2mm}
                    \begin{equation*}
                        dE_{\rm tot} = 0 \rightarrow d [ \Gamma (M_0 + m) c^2 + \Gamma_{\rm eff}E_{\rm int}' ] = \textcolor{red}{\Gamma_{\rm CBM}} dm c^2 + \Gamma_{\rm eff} dE_{\rm rad}'
                    \end{equation*}
%                    with $\rho_{\rm ISM}=f(R)$, $\Gamma_{\rm ISM}=f(R)$ (using Sedov-Taylor profile)
                    \vspace{-8mm}
                    \begin{itemize}
                        \item GRB ejecta: $\{E_{i, 0},\Gamma_{i,0} \} = f(\theta)$; %spreading: $d\theta_{\rm jet}/dR=f(c_{s})$
                        \item kN ejecta: $\{E_{i, 0},\Gamma_{i,0} \}=f(\Gamma,\theta)$, with $\rho_{\rm CBM},\Gamma_{\rm CBM}=f(R)$
                        \item GRB ejecta laterally expands $\{d\theta_i/dR_i\}$
                        \item GRB ejecta alters ISM
...
% =============================================================================================
\section{Results}%{Thermal electrons}
\begin{frame}{}  %% ---------- Intro/motivation 
    \begin{tikzpicture}[overlay,remember picture]
        \uncover<1->{ % <-> |
        ...
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-4.cm,2.5cm)}]current page.center){
                \parbox{0.54\textwidth}{
                    \textbf{Dynamics:}
                    \begin{itemize}
                        \item \textbf{Some} kilonova ejecta elements ``break through'' GRB ejecta but on various timescales; 
...
\begin{frame}{}  %% ---------- Intro/motivation 
    \begin{tikzpicture}[overlay,remember picture]
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-2.5cm,2.2cm)}]current page.center){
                \parbox{0.75\textwidth}{
                    \textbf{Very fast tail in very high resolution BNS simulations} \\
                    \textcolor{gray}{(\footnotesize{Fujibayashi S. et al., 2022, 2205.05557)}.}
                    \begin{itemize}
                        \item At lower densities, off-axis emission resembles GRB170817A
                        \item GRB flux centroid can break degeneracy
                        %                        \item for subsonic flow -- no shock, no emission $\rightarrow$ "Dip" in $F_{\nu}$;
                        %                        \item at "catch-up" -- excess in $F_{\nu}$;
                        %                        \item excess is "smeared over time" due to \\(i) kilonova ejecta structure, \\(ii) GRB lateral expansion.
                    \end{itemize}
        ...
        \uncover<1-1>{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(-4.6cm,-1.55cm)}]current page.center){
                \parbox{0.5\textwidth}{
                    \includegraphics[width=0.59\textwidth]{../figs/Fujibayashi22_DynEjectaBNS_FastTailPlot.pdf}
            }};
        }
        \uncover<1->{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(3.6cm,-1.6cm)}]current page.center){
                \parbox{0.5\textwidth}{
                    \includegraphics[width=0.55\textwidth]{../figs/lcs_preliminary.pdf}
            }};
        }
    ...
    \begin{tikzpicture}[overlay,remember picture]
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-0.7cm,3.0cm)}]current page.center){
                \parbox{1.\textwidth}{
                    \textbf{Preliminary result: radio image.} \\
                    Sky map $\{ \text{shape}, $\text{brightness}$ \} = f(\{E^{\rm kn}_{i, 0},\Gamma^{\rm kn}_{i,0} \})$. Dim. Require close source.
                    %                    \begin{itemize}
        ...
        \uncover<1-1>{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(-1.8cm,-0.9cm)}]current page.center){
                \parbox{0.9\textwidth}{
                    \includegraphics[width=1.15\textwidth]{../figs/int_BLh_M13641364_M0_LK_SR_times220_2500_skymaps_ejecta_nlayers80_theta45_nism000031_p205_epse01_epsb001_epst1_dl00413.pdf}
                    %\small{\textbf{Artist depiction of ejecta$^\text{\citep{Ascenzi:2020xqi}}$}}
            }};
        }
        \uncover<1->{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.7cm,3.75cm)}]current page.center){
                \parbox{0.35\textwidth}{
                    \textcolor{gray}{\footnotesize{VN+ (in prep.) }}
            }};
        }
    \end{tikzpicture}
\end{frame}
