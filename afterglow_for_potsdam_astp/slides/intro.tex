...
%\section{Phsyics}
\subsection{Motivation for detailed kilonova afterglow modelling}
\begin{frame}{}  %% ---------- Intro/motivation 
    \begin{tikzpicture}[overlay,remember picture]
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.5cm,-0.5cm)}]current page.center){
                \parbox{0.6\textwidth}{
                    ...
                    \begin{itemize}
                        ...
                        Harder radio-to-X-ray spectrum (lower $p$)
                        \item Observations in radio suggest optically thin spectrum
                    \end{itemize}
            }};
        }
        \uncover<1-1>{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.0cm,2.0cm)}]current page.center){
                \parbox{0.5\textwidth}{
                    \includegraphics[height=3.0cm]{figures/figure4_2021paper.pdf}
            }};
        }
        \uncover<1-1>{ % <-> |
            \node (img1) [anchor=center,scale=1,opacity=1] at ([shift={(4.0cm,-2.0cm)}]current page.center){
                \parbox{0.5\textwidth}{
                    \includegraphics[height=3.0cm]{figures/SED1234days.eps}
            }};
        }
    \end{tikzpicture}
\end{frame}
        ...
        \uncover<1->{ % <-> |
            \node (t1) [anchor=center,scale=1,opacity=1] at ([shift={(-3.1cm,-0.5cm)}]current page.center){
                \parbox{0.68\textwidth}{
                    ...
                    \textbf{Observables of the afterglow}: 
                    \begin{itemize}
...
%                    \textbf{Main Features}:
                    ...
                    \textbf{If GRB170817A change in afterglow is the emergence of a new component}: 
                    \begin{itemize}
